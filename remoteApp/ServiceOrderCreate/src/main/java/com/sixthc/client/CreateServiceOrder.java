...
public class CreateServiceOrder {
	...
	public void send(String url, String dataXmlString) throws JAXBException {
		...
		if (OrderData.isSet(od.getTaskKind()) == true
			...
			if (OrderData.isSet(od.getCrewETA()) == true) {
				SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy HH:mm");
				try {
					Date d = sdf.parse(od.getCrewETA());
					xdate = getXMLDate(d);
				} catch (ParseException ex) {
					log.info("bad crew eta format, using sysdate");
				}
			}
			
			if(xdate == null )
				xdate = getXMLDate(new java.util.Date(System.currentTimeMillis()));

			wt.setCrewETA(getXMLDate(new java.util.Date(System
					.currentTimeMillis())));
