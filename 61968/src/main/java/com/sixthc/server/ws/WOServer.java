...
public class WOServer implements WOServerSoap {
	...
	private void parseIdentiedObject(IdentifiedObject from,
		...
		if (from != null && to != null) {
			...
			to.setDescripton(from.getDescription());
	...
	private void parseContact(MspPerson from, Contactperson to) {
		...
		// .getIdentifierName());  bb: ignore
		//parseIdentiedObject(from.getIdentifiedObject(),
		//		to.getIdentifiedObject());  bb: ignore
	...
	public void initiateWorkRequest(ArrayOfWorkRequest arrayOfWorkRequest,
		...
		// expirationTime; // TODO : Not persisted bb: ignore
		// transactionID; // TODO : Not persisted bb: ignore
		...
		for (WorkRequest request : a) {
			...
			if (request.getWorkTypeRef() != null)
				workOrder
						.setKind(request.getWorkTypeRef().getWorkSubTypeName()); // TODO : Bill, verify? bb: correct

			workOrder.setWorkOrderName(request.getTitle());
			workOrder.setMrid("1234"); // auto-generated by db bb: need to generate a UUID here
			workOrder.setStatus("TBD"); // no MS equivilent bb: use "ACTIVE"
