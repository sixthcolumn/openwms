...
import ch.iec.tc57._2011.meterservicerequests.SendMeterServiceRequests;
...
public class SendMeterServiceRequestsClient extends RequestClient {
	private static org.apache.log4j.Logger log = Logger
			.getLogger(SendMeterServiceRequestsClient.class);
	...
	URL wsdlURL = ReplyMeterServiceRequestsClient.class
			.getResource("/resources/wsdl/SendMeterServiceRequests.wsdl");
	...
	public SendMeterServiceRequestsClient() {
		super();
	}
	...
	public void create() throws FaultMessage {
		...
		log.info("Created(MeterServiceRequest) client");

		log.debug("sendCreated called");
		log.debug("wsdlURL " + wsdlURL);
		log.debug("SERVICE_NAME " + SERVICE_NAME);
		SendMeterServiceRequests ss = new SendMeterServiceRequests(wsdlURL,
				SERVICE_NAME);
		SendMeterServiceRequestsPort port = ss
				.getSendMeterServiceRequestsPort();
	...
	public void execute(String cmd) throws FaultMessage {
		...
		switch (command) {
		case CANCELED:
			port.canceledMeterServiceRequests(header, payload, reply);
			break;
		case CREATED:
			port.createdMeterServiceRequests(header, payload, reply);
			break;
		case CHANGED:
			...
			break;
		case CLOSED:
			port.closedMeterServiceRequests(header, payload, reply);
			break;
		case DELETED:
			port.deletedMeterServiceRequests(header, payload, reply);
			break;
		default:
